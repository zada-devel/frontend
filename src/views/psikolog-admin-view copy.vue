  ...
  <LandingComponent>
    ...
    <div class="container my-4">
      ...
      <div v-else>
        ...
        <DataTable
          v-if="formattedPsikologs.length > 0"
          :value="formattedPsikologs"
          paginator
          :rows="25"
        ...
        >
          ...
          <!-- Menampilkan Jadwal Online -->
          <Column header="Jadwal Online">
            <template #body="{ data }">
              <span class="schedule-cell">{{ data.jadwalOnline }}</span>
            </template>
          </Column>

          <!-- Menampilkan Jadwal Offline -->
          <Column header="Jadwal Offline">
            <template #body="{ data }">
              <span class="schedule-cell">{{ data.jadwalOffline }}</span>
            </template>
          </Column>
          ...
          <Column header="Actions">
            <template #body="{ data }">
              <Button icon="pi pi-pencil" class="p-button-sm p-button-primary" @click="openEditModal(data.id)" label="Edit" />
            </template>
          </Column>
        </DataTable>
        <!-- <EasyDataTable
